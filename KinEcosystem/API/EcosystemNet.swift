...
class EcosystemNet {
    ...
    fileprivate lazy var signInData: SignInData = {
    ...
    func offers() -> Promise<Data> {
        let p = Promise<Data>()
        authorize().then {
                self.buildRequest(path: "offers", method: .get)
            }.then { request in
    ...
    func orders() -> Promise<Data> {
        let p = Promise<Data>()
        authorize().then {
            self.buildRequest(path: "orders", method: .get)
            }.then { request in
                self.request(request)
            }.then { data in
                p.signal(data)
    ...
    func authorize() -> Promise<Void> {
        ...
        guard let data = try? JSONEncoder().encode(signInData) else {
            return p.signal(EcosystemNetError.requestBuild)
        }
        buildRequest(path: "users", method: .post, body: data)
            .then { request in
                self.request(request)
            }.then { data in
    ...
    func request(request: URLRequest) -> Promise<Void> {
        let p = Promise<Void>()
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            if let error = error {
                p.signal(EcosystemNetError.network(error))
